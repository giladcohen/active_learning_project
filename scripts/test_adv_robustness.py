...
# net        = net.to(device)
net_logits = net_logits.to(device)
...
net_logits.load_state_dict(global_state['best_net'])

criterion = nn.CrossEntropyLoss()
optimizer = optim.SGD(
    net_logits.parameters(),
    lr=train_args['lr'],
...
# net.eval()
net_logits.eval()
classifier = PyTorchClassifier(model=net_logits, clip_values=(0, 1), loss=criterion,
...
test_acc = np.sum(test_preds == test_labels) / test_size
...
if args.targeted:
    ...
    if not os.path.isfile(tgt_file):
        y_test_targets = random_targets(np.asarray(test_labels), len(classes))  # .argmax(axis=1)
...
adv_accuracy = np.sum(test_adv_preds == test_labels) / test_size
