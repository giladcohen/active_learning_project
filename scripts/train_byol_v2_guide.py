"""
Same as train_byol_vw, but using only TTAs which has the correct prediction label"""
import torch
...
def train(set):
    ...
    guided_inputs_tmp = torch.empty(train_loader.dataset.data.shape, dtype=torch.float32, device=device)

    for step in range(args.steps):
        pred_correct_cnt = 0
        while pred_correct_cnt < guided_inputs_tmp.size(0):
            (inputs, targets) = list(train_loader)[0]
            inputs, targets = inputs.to(device), targets.to(device)
            y_tta_gt = targets[0].argmax()
            y_tta_preds = net(inputs)['probs'].argmax(dim=1)
            for i in range(inputs.size(0)):
                if y_tta_preds[i] == y_tta_gt:
                    guided_inputs_tmp[pred_correct_cnt] = inputs[i]
                    pred_correct_cnt += 1
                    if pred_correct_cnt == guided_inputs_tmp.size(0):
                        break

        optimizer.zero_grad()
        t_out = net(guided_inputs_tmp)
        s_out = sfs_net(guided_inputs_tmp)
